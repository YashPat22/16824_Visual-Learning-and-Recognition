...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        y = torch.bmm(self.dropout(torch.nn.functional.softmax(dot_product, dim=-1)), value)
        return y  
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # divide = 
        dot_product = torch.matmul(query, key.transpose(2, 3))/torch.sqrt(self.embed_dim)
        ...
        # apply softmax, dropout, and use value
        y = torch.matmul(self.dropout(torch.nn.functional.softmax(dot_product, dim=-1)), value)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        output = x + self.encoding(torch.arange(0, S).long().unsqueeze(0).repeat(N, 1).to("cuda"))
