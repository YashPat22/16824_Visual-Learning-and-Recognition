...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def forward(self, images: torch.Tensor):
        ...
        p5_upsampled = F.interpolate(p5, scale_factor=2)
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.pred_cls = nn.Conv2d(stem_channels[-1], self.num_classes, kernel_size=3, stride=1, padding=1) #None  # Class prediction conv
        self.pred_box = nn.Conv2d(stem_channels[-1], 4, kernel_size=3, stride=1, padding=1) #None  # Box regression conv
        self.pred_ctr =  nn.Conv2d(stem_channels[-1], 1, kernel_size=3, stride=1, padding=1) #None  # Centerness conv
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level in feats_per_fpn_level:
            stem_cls_op = self.stem_cls(feats_per_fpn_level[level])

            cls = self.pred_cls(stem_cls_op).reshape(stem_cls_op.shape[0],self.num_classes,-1).permute(0,2,1)
            center = self.pred_ctr(stem_cls_op).reshape(stem_cls_op.shape[0],1,-1).permute(0,2,1)

            stem_box_op = self.stem_box(feats_per_fpn_level[level])
            box = self.pred_box(stem_box_op).reshape(stem_box_op.shape[0],4,-1).permute(0,2,1)

            class_logits[level] = cls
            boxreg_deltas[level] = box
            centerness_logits[level] = center
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        backbone_feats = self.backbone(images.to(device="cuda"))
        pred_cls_logits, pred_boxreg_deltas, pred_ctr_logits = self.pred_net(backbone_feats)
        # pred_cls_logits, pred_boxreg_deltas, pred_ctr_logits = pred_cls_logits.to(device="cuda"), pred_boxreg_deltas.to(device="cuda"), pred_ctr_logits.to(device="cuda")
        ...
        shapes = {}
        shapes["p3"] = backbone_feats["p3"].shape
        shapes["p4"] = backbone_feats["p4"].shape
        shapes["p5"] = backbone_feats["p5"].shape

        locations_per_fpn_level = get_fpn_location_coords(shapes, shape_dict, dtype = images.dtype ,device="cuda")
        ...
        matched_gt_boxes = []
        for gt in gt_boxes:
            matched_gt_boxes.append(fcos_match_locations_to_gt(locations_per_fpn_level, shape_dict, gt))
        ...
        for gt_box in matched_gt_boxes:
            ...
            for l, box in gt_box.items():
                deltas[l] = fcos_get_deltas_from_locations(locations_per_fpn_level[l], box, shape_dict[l])
        ...
        matched_gt_boxes = matched_gt_boxes.to(device="cuda")
        matched_gt_deltas = matched_gt_deltas.to(device="cuda")

        pred_cls_logits = pred_cls_logits.to(device="cuda")
        pred_boxreg_deltas = pred_boxreg_deltas.to(device="cuda")
        pred_ctr_logits = pred_ctr_logits.to(device="cuda")

        # Perform EMA update of normalizer by number of positive locations.
        num_pos_locations = num_pos_locations.to(device="cuda")
        # pos_loc_per_image = pos_loc_per_image.to(device="cuda")
        # self._normalizer = self._normalizer.to(device="cuda")
        ...
        tgt = matched_gt_boxes[:,:,-1].long()
        tgt = torch.where(tgt == -1, 20, tgt)
        tgt = tgt.to(device="cuda")

        class_num += 1

        encode = torch.nn.functional.one_hot(tgt, class_num)

        encode = encode[:,:,:-1].float()

        encode = encode.to(device="cuda")

        loss_cls = sigmoid_focal_loss(pred_cls_logits, encode)

        loss_box = torch.nn.functional.l1_loss(pred_boxreg_deltas, matched_gt_deltas, reduction= "none")/4
        loss_box[matched_gt_deltas<0] *= 0.0

        matched_gt_deltas = matched_gt_deltas.view(-1,4)
        pred_ctr_logits = pred_ctr_logits.view(-1)
        matched_gt_centres = fcos_make_centerness_targets(matched_gt_deltas)
        matched_gt_centres = matched_gt_centres.to(device="cuda")

        loss_ctr = torch.nn.functional.binary_cross_entropy_with_logits(pred_ctr_logits, matched_gt_centres, reduction="none")
        loss_ctr[matched_gt_centres<0] *= 0.0
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_pred_classes = torch.zeros(level_pred_scores.shape[0], 1,device = "cuda")
            new_level_pred_scores = torch.zeros(level_pred_scores.shape[0], 1,device = "cuda")
            ...
            for i in range(level_pred_scores.shape[0]):
                if torch.max(level_pred_scores[i]) > test_score_thresh:
                    level_pred_classes[i][0] = torch.argmax(level_pred_scores[i]) 
                else: 
                    level_pred_classes[i][0] = -1

                if torch.max(level_pred_scores[i]) > test_score_thresh:
                    new_level_pred_scores[i][0] = torch.max(level_pred_scores[i])  
                else:
                    new_level_pred_scores[i][0] = -1
            ...
            level_pred_scores = new_level_pred_scores.view(-1,)

            
            level_pred_boxes = fcos_apply_deltas_to_locations(level_deltas, level_locations, shape_dict[level_name])

            # Step 4: Use `images` to get (height, width) for clipping.
            # Replace "pass" statement with your code
            # pass

            for i in range(level_pred_boxes.shape[0]):
                level_pred_boxes[i][0] = max(0,level_pred_boxes[i][0])
                level_pred_boxes[i][1] = max(0,level_pred_boxes[i][1])
                level_pred_boxes[i][2] = min(images.shape[2],level_pred_boxes[i][2])
                level_pred_boxes[i][3] = min(images.shape[3],level_pred_boxes[i][3])
        ...
        # print("Pred Boxes: ", pred_boxes_all_levels)
        return (
            pred_boxes_all_levels,
            pred_classes_all_levels,
            pred_scores_all_levels,
