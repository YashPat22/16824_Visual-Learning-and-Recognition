...
def train_detector(
    ...
    for _iter in trange(max_iters):
        ...
        # print('images to device', round(time.time()-start, 4))

        # start = time.time()
        gt_boxes = gt_boxes.to(device)
        # print('gt_boxes to device', round(time.time()-start, 4))

        # Dictionary of loss scalars.
        # start =  time.time()
        losses = detector(images, gt_boxes)
        # print('losses', round(time.time()-start, 4))

        # Ignore keys like "proposals" in RPN.
        # start =  time.time()
        losses = {k: v for k, v in losses.items() if "loss" in k}
        # print('losses_dict', round(time.time()-start, 4))
    ...
    torch.save(detector.state_dict(), "fcos_detector-2.pt")
...
def inference_with_detector(
    ...
    if output_dir is None:
        writer=SummaryWriter("detection_logs_test")
