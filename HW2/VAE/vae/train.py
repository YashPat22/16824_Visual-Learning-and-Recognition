...
def ae_loss(model, x):
    ...
    # loss = torch.nn.MSELoss(decode, x, reduction ='mean')
    # loss = F.mse_loss(decode, x, reduction='mean')
    loss = F.mse_loss(decode, x, reduction='sum')
    loss = loss/x.size(0)
...
def vae_loss(model, x, beta = 1):
    ...
    decode = model.decoder(z)

    recon_loss = F.mse_loss(decode, x, reduction='sum')
    
    recon_loss = recon_loss/x.size(0)

    kl_loss = -0.5*(torch.sum(1+(2*var)-mu.pow(2)-(2*var).exp()))/x.size(0)
...
def linear_beta_scheduler(max_epochs=None, target_val = 1):
    ...
    ##################################################################
    def _helper(epoch):
        ret = (epoch*target_val)/(max_epochs)
        return ret
        # pass
    ##################################################################
    #                          END OF YOUR CODE                      #
...
def main(log_dir, loss_mode = 'vae', beta_mode = 'constant', num_epochs = 20, batch_size = 256, latent_size = 256,
    ...
    os.makedirs('/home/ubuntu/hw2/generative-modeling/data'+ log_dir, exist_ok = True)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1)%eval_interval == 0:
            ...
            vis_recons(model, vis_x, '/home/ubuntu/hw2/generative-modeling/data'+log_dir+ '/epoch_'+str(epoch))
            if loss_mode == 'vae':
                vis_samples(model, '/home/ubuntu/hw2/generative-modeling/data'+log_dir+ '/epoch_'+str(epoch) )
    for k,v in plot_metrics.items():
        plt.clf()
        save_plot(list(range(len(v))), v, "Epochs", k, f"{k} vs. Epochs", '/home/ubuntu/hw2/generative-modeling/data' + log_dir + f'/{k}_vs_iterations')
