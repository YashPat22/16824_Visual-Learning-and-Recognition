...
def get_optimizers_and_schedulers(gen, disc):
    ...
    # 100K iterations.
    ##################################################################
    # lambda_disc = lambda epoch : 1 - (epoch/500000)
    scheduler_discriminator = torch.optim.lr_scheduler.LambdaLR(optim_discriminator, lr_lambda=lambda epoch : 1 - (epoch/500000))
    # lambda_gen = lambda epoch : 1 - (epoch/100000)
    scheduler_generator = torch.optim.lr_scheduler.LambdaLR(optim_generator, lr_lambda=lambda epoch : 1 - (epoch/100000))
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
    return (
...
def train_model(
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        Dataset(root="/home/ubuntu/hw2/generative-modeling/datasets/CUB_200_2011_32", transform=ds_transforms),
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                ##################################################################
                # print("Train Batch: ",train_batch.size()[0])
                # generator_output = gen.forward(n_samples = train_batch.size()[0])
                ...
                interp = ((1-alpha) * train_batch) + ((alpha)*generator_output)
                # interp = ((1-alpha) * generator_output) + ((alpha)*generator_output)
